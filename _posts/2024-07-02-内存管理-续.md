...
b. 如果pte被设置了allocate from zero，也就是需要给这段虚拟地址分配实际的物理页，并清零，pte的这个标志应该是需要分配新的虚拟地址空间的时候去设置，暂时还没看到有地方去设置这个  

最后一步就是将这个pte添加到HPTE中，最终还是调用wirtePTE
```c++
